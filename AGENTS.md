...
**Mode:** evidence-gated build-out. Five composable v2 artifacts are built and mock-tested
(run-ledger, merge-gate, plan-preflight + spec-interview, heldout-suite, exec-loop with its
launchers — see [tools/](tools/)); the design surface remains the authority for every change:
[docs/design/evidence-based-harness.md](docs/design/evidence-based-harness.md), with the
evidence base at [docs/research/distilled/](docs/research/distilled/README.md). The repo was
...
- **Git discipline:** never commit directly to `main` — feature branch → commit → `--ff-only`
  merge → delete branch.
- **Quota discipline:** operations that spend real quota (any vendor — Max-plan windows,
  OpenAI credits) are operator-run or operator-directed, never wired into loops, gates, tests,
  or CI. **Never run** `tools/budget-governor/run_cache_weight_experiment.sh`
  `dry-run`/`arm-a`/`arm-b` (`gen-filler`/`summarize` are free and fine).
