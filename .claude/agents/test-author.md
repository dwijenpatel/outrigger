...
6. **Discharge what your corpus already absorbs.** When you deliberately wrote the tests to pass under *every* reading of an ambiguity (pinning the behavior, not the literal choice), record that entry as `{"text": "...", "corpus_covers": "both"}` — it stays on the record but never parks the task, because no reading the implementer picks can produce validated-wrong software. Use the object form with `"both"` only when that is literally true for your authored tests; anything the corpus pins to one reading stays a plain string (blocking on high/critical). Don't resolve an ambiguity by silently picking a reading — either cover both or surface it.
