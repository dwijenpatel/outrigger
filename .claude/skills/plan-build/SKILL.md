...
- `plan/tasks.json` — the ledger (schema: `harness/ledger.py` `validate_tasks`; fields: `id`, `phase`, `profile`, `deps`, optional `may_be_invalidated_by`, **`regime`** — tag every task `chore` (tool-loop mechanics: run tests, small fixes, seeds), `thinking` (design/algorithmic work), or `long_horizon` (multi-hour, repo-spanning): it conditions the implementer's tier (I18). When unsure between two regimes, pick the heavier one — the only expensive routing error is sending thinking/long work down). Phase 1 = a small walking skeleton (4–6 tasks is fine); **small must not mean vague**.
...
- **Record `touches` per task (I19).** Give every task a `touches` list in `tasks.json`: the representative *concrete paths* its spec pins (files it creates or edits — e.g. `app/models.py`, not a glob). This is what lets the preflight below prove floors×profiles mechanically instead of by eyeball; P2-13's reconciliation stops being a discipline and becomes a check.
- `plan/conventions.json` — **pin the project's run conventions (P3v2-4)**: at minimum `{"heldout_cmd": "<the exact command the gate runs against the materialized `.heldout` corpus in a clean checkout>"}`. Interview for it explicitly (test framework, fixture/conftest needs, PYTHONPATH shape) — pilot-3-v2 discovered its convention mid-firing via 14 fixture errors; the firing must read it from the plan, not rediscover it.

## Ratification (hard stop)

0. **Preflight first (I19, mandatory — and again on ANY re-ratification):** `python3 -m harness.planning preflight --plan-dir plan --vault-config harness/config/vault-isolation.json`. This simulates every statically-evaluable pre-spawn gate: floors×touches collisions and — critically on re-ratifications and vault-carrying reruns — H9 ambiguity parks armed by profile raises against *existing* handoffs (both pilot-3 halts were foreseeable this way; one cost an 8h25m mid-firing stall). Surface every finding to the operator **as part of this ratification conversation** and adjudicate each (plan amendment, or a resolved blocker card under `state/blockers/`) before presenting for approval. A question answerable now, while the operator is in the room, must never be deferred to a mid-firing ask.
1. Present the plan: a digest of decisions, the task table with profiles, where each risk lives, and the preflight result with any adjudications. **STOP and wait for explicit approval.** Silence, partial answers, or "looks fine so far" are not approval.
