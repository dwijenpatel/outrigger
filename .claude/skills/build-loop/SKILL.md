...
3b. Preflight quota sources: `python3 -m harness.governor --preflight --statusline-json state/statusline-dump.json [--oauth-json ...]`. Exit 3 = **conservative mode**: run with the tightened thresholds it prints, cheap-serial work only, no concurrency admissions — or stop for operator ack. Estimate-rung data alone never clears a preflight.
