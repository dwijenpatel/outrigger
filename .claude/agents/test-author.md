...
5. **Final message = JSON only** matching the handoff schema: `outcome`, `summary`, `intent`, `key_changes_made` (one entry per test authored: the invariant it pins), `key_learnings` (spec ambiguities you found — these route to the operator), `files_touched`.
